...
export const RemedialList = ({ remedials, group, category, onBack }: Props) => {
  const handleStatusChange = (id: string, status: RemedialRequest["status"], instructions?: string) => {
    ...
    dbConnect()?.editDocument(Collections.CATEGORIES, category.id, category).then((res) => {
      if (res) alert("Nivelación actualizada");
    });
  };
        ...
        <ul style={{ marginTop: "1rem" }}>
          {remedials.map((r) => (
            <li key={r.id} style={{ ...styles.card, marginBottom: "1rem" }}>
              <p><strong>Estudiante:</strong> {r.student}</p>
              <p><strong>Fecha de solicitud:</strong> {new Date(r.requestedAt).toLocaleString()}</p>
              <p><strong>Estado:</strong> {r.status}</p>
              {r.instructions && <p><strong>Instrucciones:</strong> {r.instructions}</p>}
              {r.studentResponse && <p><strong>Respuesta del estudiante:</strong> {r.studentResponse}</p>}
              ...
              {r.status === "pendiente" && (
                <div style={{ marginTop: "0.5rem" }}>
                  ...
                  <button
                    style={{ ...styles.button, backgroundColor: "#ef4444", marginLeft: "0.5rem" }}
              ...
              {r.status === "aceptada" && !r.resolvedAt && (
                <button
                  style={{ ...styles.button, backgroundColor: "#3b82f6", marginTop: "0.5rem" }}
